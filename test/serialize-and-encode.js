/* eslint-disable prefer-rest-params, require-yield */
...
test(useDeserialized, Number.NaN);
test(useDeserialized, Number.POSITIVE_INFINITY);
test(useDeserialized, Number.NEGATIVE_INFINITY);
test('-0x80 (int8)', useDeserialized, -0x80);
test('0x7F (int8)', useDeserialized, 0x7F);
test('-0x8000 (int16)', useDeserialized, -0x80_00);
test('0x7FFF (int16)', useDeserialized, 0x7F_FF);
test('-0x800000 (int24)', useDeserialized, -0x80_00_00);
test('0x7FFFFF (int24)', useDeserialized, 0x7F_FF_FF);
test('-0x80000000 (int32)', useDeserialized, -0x80_00_00_00);
test('0x7FFFFFFF (int32)', useDeserialized, 0x7F_FF_FF_FF);
test('-0x8000000000 (int40)', useDeserialized, -0x80_00_00_00_00);
test('0x7FFFFFFFFF (int40)', useDeserialized, 0x7F_FF_FF_FF_FF);
test('-0x800000000000 (int48)', useDeserialized, -0x80_00_00_00_00_00);
test('0x7FFFFFFFFFFF (int48)', useDeserialized, 0x7F_FF_FF_FF_FF_FF);
test('-0x800000000001 (larger than int48)', useDeserialized, -0x8_00_00_00_00_01);
test('0x800000000000 (larger than int48)', useDeserialized, 0x80_00_00_00_00_00);
...
if (typeof BigInt === 'function') {
	test('42n', useDeserialized, BigInt(42));
}
...
test('object with length property', useDeserialized, {length: 12_345_678});
test('object with negative length property', useDeserialized, {length: -12_345_678});
test('object with NaN length property', useDeserialized, {length: Number.NaN});
test('object with infinite length property', useDeserialized, {length: Number.POSITIVE_INFINITY});
...
// Pointers
{
	const object = {};
	object.self = object;
	test('object with pointer to itself', useDeserialized, object);
}

// Other complex values
test('arguments', useDeserialized, (function () {
	return arguments;
})('foo', {}));
{
