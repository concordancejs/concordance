import {cloneDeep, merge} from './lodash/index.js';
...
function normalizePlugins(plugins) {
	...
	const theme = freezeTheme(themes.reduce((acc, pluginTheme) => merge(acc, pluginTheme), cloneDeep(defaultTheme)));
...
export function normalize(options) {
	options = {plugins: [], theme: null, ...options};
...
export function applyModifiers(descriptor, theme) {
	if (!modifiers.has(descriptor)) {
		return theme;
	}

	// eslint-disable-next-line unicorn/no-array-reduce
	return [...modifiers.get(descriptor)].reduce((previous, modifier) => {
		...
		if (cache.has(previous)) {
			return cache.get(previous);
		}

		const modifiedTheme = cloneDeep(previous);
		modifier(modifiedTheme);
		freezeTheme(modifiedTheme);
		cache.set(previous, modifiedTheme);
		originalCache.set(modifiedTheme, theme);
		return modifiedTheme;
	}, theme);
}

export function applyModifiersToOriginal(descriptor, theme) {
