...
export function consumeDeep(recursor) {
	const stack = [recursor];
	while (stack.length > 0) {
		const subject = stack.at(-1)();
		if (subject === null) {
			stack.pop();
			continue;
		}
...
export function map(recursor, mapFn) {
	return () => {
		const next = recursor();
		if (next === null) {
			return null;
...
export function replay(state, create) {
	if (!state) {
...
export function sequence(first, second) {
	let fromFirst = true;
	return () => {
		if (fromFirst) {
			const next = first();
...
export function unshift(recursor, value) {
	return () => {
		if (value !== null) {
			const next = value;
			value = null;
