...
import * as object from './complexValues/object.js';
...
import * as stringDescriptor from './primitiveValues/string.js';
...
export function add(plugin) {
	...
	const addDescriptor = (id, tag, deserialize) => {
		if (id2deserialize.has(id)) {
			throw new DuplicateDescriptorIdError(id, plugin);
		}

		if (descriptorRegistry.has(tag) || tag2id.has(tag)) {
			throw new DuplicateDescriptorTagError(tag, plugin);
		}
...
export function getDeserializers(plugins) {
...
export function getThemes(plugins) {
	return plugins.map(plugin => {
		const registered = add(plugin);
		return {
			name: registered.name,
...
export function getTryDescribeValues(plugins) {
	return plugins.map(plugin => add(plugin).tryDescribeValue);
}

export function resolveDescriptorRef(tag) {
