...
class MapEntry {
  ...
  format (_, indent) {
    ...
    return {
      ...
      finalize () {
        ...
        // the key. Unfortunately that needs to happen in MapComplex. Prefix
        // each value with the key, then rely on MapComplex to correctly format
  ...
  formatDiff (expected, gutters, indent) {
    ...
    if (expected.isMapEntry !== true) return null
  ...
  rebalanceDiff (recursor, expectedRecursor, expected, compareComplexShape) {
    ...
    while (!expectedIsMissing && expected !== null && expected.isMapEntry === true) {
    ...
    if (initialExpected.isMapEntry === true) {
      if (initialExpected.keyIsPrimitive) {
        let actual = this
        while (!actualIsExtraneous && actual !== null && actual.isMapEntry === true) {
