...
function diffLine (actual, expected, gutters, open, close, lastOfActual, lastOfExpected) {
  ...
  if (
    ...
    return [
      { actual: true, last: lastOfActual, gutter: gutters.actualIsWrong, formatted: actual },
      { expected: true, last: lastOfExpected, gutter: gutters.wasExpected, formatted: expected }
    ]
  }
  ...
  return diffResults.reduce((acc, result) => {
    ...
    { actual: true, last: lastOfActual, gutter: gutters.actualIsWrong, formatted: '' },
    { expected: true, last: lastOfExpected, gutter: gutters.wasExpected, formatted: '' }
  ])
}
...
class StringValue {
  ...
  formatDiff (expected, gutters, indent) {
    ...
    if (!includesLinebreaks(escapedActual) && !includesLinebreaks(escapedExpected)) {
      return diffLine(
        formatWithoutLinebreaks(escapedActual),
        formatWithoutLinebreaks(escapedExpected),
        gutters,
        '\'', '\'',
        true, true)
    }
    ...
    for (let actualIndex = 0, expectedIndex = 0, extraneousOffset = 0; actualIndex < actualLines.length;) {
      if (actualLines[actualIndex] === expectedLines[expectedIndex]) {
        lines.push({
          neutral: true,
          last: actualIndex === lastActualIndex && expectedIndex === lastExpectedIndex,
          gutter: gutters.neutral,
          formatted: actualLines[actualIndex]
        })
        actualIndex++
        expectedIndex++
        continue
      ...
      if (actualIsExtraneous && !expectedIsMissing) {
        lines.push({
          actual: true,
          last: actualIndex === lastActualIndex,
          gutter: gutters.actualIsExtraneous,
          formatted: actualLines[actualIndex]
        })
        actualIndex++
        extraneousOffset++
        ...
        lines.push({
          expected: true,
          last: expectedIndex === lastExpectedIndex,
          gutter: gutters.expectedIsMissing,
          formatted: expectedLines[expectedIndex]
        })
        expectedIndex++
      } else {
