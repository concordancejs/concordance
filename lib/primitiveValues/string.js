...
function escapeInvisibleAndAmbiguous (char) {
  if (char === '\n' || char === '\r') return char
  if (char === '\t') return '\\t'
  if (char === '\b') return '\\b'
  if (char === '\f') return '\\f'
  ...
  // Plus control and formatting code points that can be hard to spot.
...
function basicEscape (string) {
  return string
    .replace(/\\/g, '\\\\')
    .replace(/[\s\S]/g, escapeInvisibleAndAmbiguous)
}
...
class StringValue {
  ...
  formatAsKey (theme) {
    ...
    if (escaped === key && (keyword.isIdentifierNameES6(key, true) || String(parseInt(key, 10)) === key)) {
      return key
    }
