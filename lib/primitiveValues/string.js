...
const DIFF_MARKER_START = 0xE000

function splitWordDiffTokens (string) {
  const tokens = string.match(MATCH_WORD_DIFF_TOKEN)
  return tokens === null ? [] : tokens
}

function tokenizeForWordDiff (actual, expected) {
  const actualTokens = splitWordDiffTokens(actual)
  const expectedTokens = splitWordDiffTokens(expected)

  if (actualTokens.length < 2 || expectedTokens.length < 2) return null

  const tokens = []
  const indexes = new Map()
  const chars = []

  for (const token of actualTokens.concat(expectedTokens)) {
    if (indexes.has(token)) continue

    const index = indexes.size
    if (DIFF_MARKER_START + index > 0xF8FF) return null

    indexes.set(token, index)
    tokens.push(token)
    chars.push(String.fromCharCode(DIFF_MARKER_START + index))
  }

  return {
    actual: actualTokens.map(token => chars[indexes.get(token)]).join(''),
    expected: expectedTokens.map(token => chars[indexes.get(token)]).join(''),
    tokens,
  }
}

function diffWords (actual, expected) {
  const tokenized = tokenizeForWordDiff(actual, expected)
  if (tokenized === null) return fastDiff(actual, expected)

  return fastDiff(tokenized.actual, tokenized.expected).map(([operation, encoded]) => {
    let string = ''
    for (const char of encoded) {
      string += tokenized.tokens[char.charCodeAt(0) - DIFF_MARKER_START]
    }
...
function diffLine (theme, actual, expected, invert) {
  const outcome = diffWords(actual, expected)
