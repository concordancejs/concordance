...
export const tag = Symbol('StringValue');
...
function themeControlPictures(theme, resetWrap, string_) {
	return string_.replaceAll(MATCH_CONTROL_PICTURES, picture => resetWrap.close + formatUtils.wrap(theme.string.controlPicture, picture) + resetWrap.open);
}
...
function escapeQuotes(line, string) {
	...
	if (quote === '\'') {
		return string.replaceAll(MATCH_SINGLE_QUOTE, '\\\'');
	}

	if (quote === '"') {
		return string.replaceAll(MATCH_DOUBLE_QUOTE, '\\"');
	}

	if (quote === '`') {
		return string.replaceAll(MATCH_BACKTICKS, '\\`');
	}
...
function gatherLines(string) {
	const lines = [];
	let previousIndex = 0;
	for (let match; (match = LINEBREAKS.exec(string)); previousIndex = match.index + match[0].length) {
		lines.push(string.slice(previousIndex, match.index) + escapeLinebreak(match[0]));
	}

	lines.push(string.slice(previousIndex));
	return lines;
}
...
class StringValue {
	...
	formatDeep(theme, indent) {
		...
		const lineStrings = gatherLines(escaped).map(string => formatUtils.wrap(theme.string, themeControlPictures(theme, theme.string, string)));
		...
		return lineBuilder.buffer()
			.append(
				lineStrings.map((string, index) => {
					if (index === 0) {
						return lineBuilder.first(theme.string.multiline.start + string);
					}

					if (index === lastIndex) {
						return lineBuilder.last(indentation + string + theme.string.multiline.end);
					}
	...
	formatAsKey(theme) {
		...
		if (keyword.isIdentifierNameES6(key, true) || String(Number.parseInt(key, 10)) === key) {
			return key;
		}
	...
	diffDeep(expected, theme, indent, invert) {
		...
		if (!includesLinebreaks(escapedActual) && !includesLinebreaks(escapedExpected)) {
			...
			return lineBuilder.actual.single(formatUtils.wrap(theme.string.line, result[0]))
				.concat(lineBuilder.expected.single(formatUtils.wrap(theme.string.line, result[1]))); // eslint-disable-line unicorn/prefer-spread
		}
