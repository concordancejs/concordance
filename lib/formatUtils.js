...
export class ObjectFormatter {
	...
	finalize() {
		...
		const {ctor} = this.object;
		const {stringTag} = this.object;
		...
		return lineBuilder.first(prefix + variant.openBracket)
			.concat(this.innerLines.withFirstPrefixed(this.indent.increase()).stripFlags()) // eslint-disable-line unicorn/prefer-spread
...
export class SingleValueFormatter {
