...
import * as pointerDescriptor from './metaDescriptors/pointer.js';
import * as propertyDescriptor from './metaDescriptors/property.js';
import * as statsDescriptors from './metaDescriptors/stats.js';
...
export function serialize(descriptor) {
	...
	const resolvePluginRef = tag => {
		...
		if (!usedPlugins.has(ref.name)) {
			...
			usedPlugins.set(ref.name, {index, ...ref.serialization});
		}
...
export function deserialize(buffer, options) {
	...
	const getDescriptorDeserializer = (pluginIndex, id) => (state, recursor) => {
