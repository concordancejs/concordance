...
function formatDescriptor (subject, options) {
  ...
  do {
    ...
    if (circular.has(subject)) {
      stack[topIndex].formatter.append(lineBuilder.single(theme.circular), origin)
    } else {
      let didFormat = false
      if (typeof subject.formatDeep === 'function') {
        ...
        if (formatted !== null) {
          ...
          if (topIndex === -1) {
            ...
            stack[topIndex].formatter.append(formatted, origin)
          }
        }
      }
    ...
    while (topIndex >= 0) {
      ...
      if (topIndex === -1) {
        ...
        stack[topIndex].formatter.append(formatted, record.origin)
      }
    }
  } while (topIndex >= 0)
