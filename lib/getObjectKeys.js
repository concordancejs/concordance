...
export default function getObjectKeys(object, excludeListItemAccessorsBelowLength) {
	const keys = [];
	let size = 0;

	// Sort property names, they should never be order-sensitive
	const nameCandidates = Object.getOwnPropertyNames(object).sort();
	// Comparators should verify symbols in an order-insensitive manner if
	// possible.
	const symbolCandidates = Object.getOwnPropertySymbols(object);
	...
	for (const name of nameCandidates) {
		...
		if (accept && isEnumerable(object, name)) {
			keys[size++] = name;
		}
	}

	for (const symbol of symbolCandidates) {
		if (isEnumerable(object, symbol)) {
			keys[size++] = symbol;
		}
	}
