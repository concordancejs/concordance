...
export const descriptorSymbol = Symbol('descriptor');
...
function encodeValue(value) {
	...
	if (type === 'number') {
		if (Number.isInteger(value)) {
			...
			if (value >= -0x80_00 && value < 0x80_00) {
				return encodeInteger(valueTypes.int16, value);
			}

			if (value >= -0x80_00_00 && value < 0x80_00_00) {
				return encodeInteger(valueTypes.int24, value);
			}

			if (value >= -0x80_00_00_00 && value < 0x80_00_00_00) {
				return encodeInteger(valueTypes.int32, value);
			}

			if (value >= -0x80_00_00_00_00 && value < 0x80_00_00_00_00) {
				return encodeInteger(valueTypes.int40, value);
			}

			if (value >= -0x80_00_00_00_00_00 && value < 0x80_00_00_00_00_00) {
...
function decodeValue(buffer, byteOffset) {
	...
	if (type === valueTypes.zero) {
		return {byteOffset, value: 0};
	}

	if (type === valueTypes.negativeZero) {
		return {byteOffset, value: -0};
	}

	if (type === valueTypes.notANumber) {
		return {byteOffset, value: Number.NaN};
	}

	if (type === valueTypes.infinity) {
		return {byteOffset, value: Number.POSITIVE_INFINITY};
	}

	if (type === valueTypes.negativeInfinity) {
		return {byteOffset, value: Number.NEGATIVE_INFINITY};
	}

	if (type === valueTypes.undefined) {
		return {byteOffset, value: undefined};
	}
	...
	if (type === valueTypes.true) {
		return {byteOffset, value: true};
	}

	if (type === valueTypes.false) {
	...
	if (type === valueTypes.numberString || type === valueTypes.utf8 || type === valueTypes.bytes || type === valueTypes.bigInt) {
		...
		if (type === valueTypes.bigInt) {
			const value = BigInt(buffer.toString('utf8', start, end));
			return {byteOffset: end, value};
		}
	...
	if (type === valueTypes.list || type === valueTypes.descriptor) {
		...
		const value = Array.from({length: length.value});
...
function buildBuffer(numberOrArray) {
	...
	const array = numberOrArray.flat(Number.POSITIVE_INFINITY);
	const buffers = Array.from({length: array.length});
	let byteLength = 0;
...
export function encode(serializerVersion, rootRecord, usedPlugins) {
	const buffers = [];
	let byteOffset = 0;
	...
	const numberPlugins = buildBuffer(encodeValue(usedPlugins.size));
	buffers.push(numberPlugins);
	byteOffset += numberPlugins.byteLength;
...
export function decodePlugins(buffer) {
	const $numberPlugins = decodeValue(buffer, 0);
	let {byteOffset} = $numberPlugins;

	const usedPlugins = new Map();
	const lastIndex = $numberPlugins.value;
...
export function decodeRecord(buffer, byteOffset) {
	...
	const $numberPointers = decodeValue(buffer, byteOffset);
	const numberPointers = $numberPointers.value;
	byteOffset = $numberPointers.byteOffset;

	const pointerAddresses = Array.from({length: numberPointers});
	for (let index = 0; index < numberPointers; index++) {
