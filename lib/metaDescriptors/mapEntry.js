...
class MapEntry {
    ...
    return {
      ...
      finalize () {
        ...
        return values.map(value => {
          const prefixed = prefixMultilineKey(keyLines, value)
          prefixed.formatted += ','
          return prefixed
        })
      }
  ...
  formatDiff (expected, gutters, indent) {
    ...
    // The following logic assumes the formatted diff came from the string primitive. If the value is of a different
    // type then fall back to format(), which should buffer the results of `this.value.format()` and
    // `expected.value.format()`.
    if (this.value.tag !== stringPrimitive) return null

    // Only use this logic to format string diffs when the keys are the same.
    ...
    prefixed[0] = prefixMultilineKey(keyLines, valueDiff[0])
    ...
    for (let index = 1; index < valueDiff.length; index++) {
      ...
      if (mustPrefixExpected && line.expected === true) {
        prefixedLine = prefixMultilineKey(keyLines, line)
        mustPrefixExpected = false
      }

      if (line.last) {
        prefixedLine.formatted += ','
      }
