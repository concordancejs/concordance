import {DEEP_EQUAL, UNEQUAL, SHALLOW_EQUAL} from '../constants.js';
import lineBuilder from '../lineBuilder.js';
import * as recursorUtils from '../recursorUtils.js';
import * as themeUtils from '../themeUtils.js';

export function describe(keyDescriptor, valueDescriptor) {
...
export const tag = Symbol('MapEntry');
...
class MapEntry {
	...
	prepareDiff(expected, lhsRecursor, rhsRecursor, compareComplexShape, isCircular) {
		...
		while (!expectedIsMissing && expected !== null && this.tag === expected.tag) {
			expectedIsMissing = expected.keyIsPrimitive ? this.key.compare(expected.key) !== UNEQUAL : compareComplexShape(this.key, expected.key) !== UNEQUAL;
		...
		if (this.tag === initialExpected.tag) {
			if (initialExpected.keyIsPrimitive) {
				let actual = this; // eslint-disable-line unicorn/no-this-assignment
		...
		if (!this.keyIsPrimitive && !initialExpected.keyIsPrimitive) {
			mustRecurse = this.valueIsPrimitive || initialExpected.valueIsPrimitive
				? this.value.compare(initialExpected.value) !== UNEQUAL
				: compareComplexShape(this.value, initialExpected.value) !== UNEQUAL;
		}
