...
class MapEntry {
  ...
  formatDiff (theme, expected, indent) {
    ...
    // Since the regular format() applies theme modifiers before formatting the
    // value, do the same here.
    const valueDiff = this.value.formatDiff(themeUtils.applyModifiersToOriginal(this.value, theme), expected.value, indent)
    ...
    const keyLines = this.key.format(themeUtils.applyModifiersToOriginal(this.key, theme), '', indent, '').split('\n')
