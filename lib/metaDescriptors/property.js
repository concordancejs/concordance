...
class ComplexProperty extends Property {
  ...
  format (theme) {
    return new formatUtils.SingleValueBuffer(value => {
      return this.key.formatAsKey(theme) + theme.property.separator + value + theme.property.after
    })
  }
...
class PrimitiveProperty extends Property {
  ...
  format (theme, gutter, indent, innerIndent) {
    return this.key.formatAsKey(theme) + theme.property.separator +
      this.value.format(theme, gutter, indent, innerIndent) +
  ...
  formatDiff (theme, expected, indent) {
    ...
    const key = this.key.formatAsKey(theme) // Assume keys are formatted into a single line
    valueDiff[0].formatted = key + theme.property.separator + valueDiff[0].formatted

    let mustPrefixExpected = valueDiff[0].isActual === true
    for (const line of valueDiff) {
      if (mustPrefixExpected && line.isExpected === true) {
        line.formatted = key + theme.property.separator + line.formatted
        mustPrefixExpected = false
      }

      if (line.isLast === true) {
        line.formatted += theme.property.after
      }
    }
