...
import {tag as symbolPrimitive} from '../primitiveValues/symbol.js'
import * as recursorUtils from '../recursorUtils.js'

export function describeComplex (key, value) {
	return new ComplexProperty(key, value)
}

export function deserializeComplex (key, recursor) {
	const value = recursor()
	return new ComplexProperty(key, value)
}

export function describePrimitive (key, value) {
	return new PrimitiveProperty(key, value)
}

export function deserializePrimitive (state) {
	const key = state[0]
	const value = state[1]
	return new PrimitiveProperty(key, value)
}

export const complexTag = Symbol('ComplexProperty')

export const primitiveTag = Symbol('PrimitiveProperty')
