...
class ComplexProperty extends Property {
  ...
  formatShallow (theme, indent) {
    ...
    return new formatUtils.SingleValueFormatter(value => {
      if (increaseValueIndent) {
        return lineBuilder.first(this.key.formatAsKey(theme) + theme.property.separator)
          .concat(value.withFirstPrefixed(indent.increase()).stripFlags())
          .append(lineBuilder.last(indent + theme.property.after))
      }
...
class PrimitiveProperty extends Property {
  ...
  formatDeep (theme, indent) {
    ...
    if (increaseValueIndent) {
      return lineBuilder.first(this.key.formatAsKey(theme) + theme.property.separator)
        .concat(formatted.withFirstPrefixed(valueIndent).stripFlags())
        .append(lineBuilder.last(indent + theme.property.after))
    }
  ...
  diffDeep (expected, theme, indent) {
    ...
    if (increaseValueIndent) {
      return lineBuilder.first(this.key.formatAsKey(theme) + theme.property.separator)
        .concat(diff.withFirstPrefixed(valueIndent).stripFlags())
        .append(lineBuilder.last(indent + theme.property.after))
    }
