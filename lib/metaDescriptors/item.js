...
class ComplexItem {
  ...
  formatShallow (theme) {
    return new formatUtils.SingleValueFormatter(value => value.withLastPostfixed(theme.item.after))
  }
...
class PrimitiveItem {
  ...
  formatDeep (theme, indent) {
    ...
    return this.value.formatDeep(theme, indent).withLastPostfixed(theme.item.after)
  }
  ...
  diffDeep (expected, theme, indent) {
    ...
    // modifiers to the item descriptor instead.
    const diff = this.value.diffDeep(expected.value, theme, indent)
