  ...
  prepareDiff (expected, lhsRecursor, rhsRecursor, compareComplexShape, isCircular) {
    ...
    do {
      ...
      if (
        ...
        lhsBuffer.shift()
        rhsBuffer.shift()
        return { compareResult, lhsRecursor: lhsFork.recursor, rhsRecursor: rhsFork.recursor }
      }
    ...
    // extraneous LHS item. Handle the latter here, deferring the former (if
    // present) to its own `prepareDiff` output.
    const extraneous = lhsBuffer.splice(0, lhsLen - 1)
    return {
      multipleAreExtraneous: true,
      descriptors: extraneous,
      lhsRecursor: lhsFork.recursor,
      rhsRecursor: withIndexIncrement(rhsFork.recursor, extraneous.length),
    }
  }
