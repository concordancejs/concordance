...
export function describeComplex(index, value) {
	return new ComplexItem(index, value);
}

export function deserializeComplex(index, recursor) {
	const value = recursor();
	return new ComplexItem(index, value);
}

export function describePrimitive(index, value) {
...
class PrimitiveItem {
	...
	prepareDiff(expected, lhsRecursor, rhsRecursor, _compareComplexShape, _isCircular) {
