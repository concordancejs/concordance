...
function diffDescriptors (lhs, rhs, options) {
  const theme = themeUtils.normalize(options && options.theme)
  ...
  const format = (gutter, subject, lookup) => {
    ...
    do {
      ...
      if (circular.has(subject)) {
        ...
        formatted = subject.format(themeUtils.applyModifiers(subject, theme), gutter, indent, innerIndent)
      }
  ...
  while (retval === undefined) {
    ...
    if (result === DEEP_EQUAL) {
      ...
      const custom = lhs.formatDiff
        ? lhs.formatDiff(themeUtils.applyModifiers(lhs, theme), rhs, indent)
        : null
    ...
    if (diffDeep) {
      ...
      if (lhs.format) {
        const formatted = lhs.format(themeUtils.applyModifiers(lhs, theme), theme.gutters.neutral, indent, innerIndent)
        if (typeof formatted === 'string') {
