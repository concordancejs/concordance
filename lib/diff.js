    ...
    if (compareResult !== DEEP_EQUAL && !firstPassSymbolProperty && typeof lhs.prepareDiff === 'function') {
      ...
      if (instructions !== null) {
        ...
        if (instructions.mustRecurse === true) {
          mustRecurse = true
        } else if (instructions.lhsDescriptors || instructions.rhsDescriptors) {
          const len = instructions.lhsDescriptors.length
          if (instructions.rhsDescriptors.length !== len) {
            throw new Error('Unmatched LHS/RHS descriptors from prepareDiff()')
          }
          diffablePairs = []
          for (let i = 0; i < len; i++) {
            diffablePairs.push([instructions.lhsDescriptors[i], instructions.rhsDescriptors[i]])
          }
          ...
          if (instructions.multipleAreExtraneous === true) {
            const descriptors = instructions.extraneousDescriptors || instructions.descriptors
            for (const extraneous of descriptors) {
              format(lineBuilder.actual, extraneous, lhsCircular)
            }
          }
          if (instructions.multipleAreMissing === true) {
            const descriptors = instructions.missingDescriptors || instructions.descriptors
            for (const missing of descriptors) {
