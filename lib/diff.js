...
function diffDescriptors (lhs, rhs) {
  ...
  while (retval === undefined) {
    ...
    if (result === DEEP_EQUAL) {
      ...
      if (custom) {
        formatStack[formatIndex].formatter.buffer(GUTTER_NEUTRAL, custom, lhs)
      } else if (rebalanced) {
        ...
        if (rebalanced.actualIsExtraneous === true) {
          ...
          for (const extraneous of rebalanced.descriptors) {
            format(GUTTERS.actualIsExtraneous, extraneous, lhsLookup)
          }
          ...
          for (const missing of rebalanced.descriptors) {
            format(GUTTERS.expectedIsMissing, missing, rhsLookup)
          }
    ...
    do {
      ...
      if (lhs === null && rhs === null) {
        ...
        if (lhsRecord.formatIndex === formatIndex) {
          ...
          if (formatted !== null) {
            formatStack[formatIndex].formatter.buffer(GUTTER_NEUTRAL, formatted, record.origin)
          }
        }
