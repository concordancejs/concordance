    ...
    return () => {
      ...
      while (!recursedList || !recursedProperty || !recursedIterable) {
        ...
        if (!recursedList && (this.isList || recursedProperty)) {
          ...
          recursedList = true
          if (recursor !== recursorUtils.NOOP_RECURSOR) return new ListStats(recursor)
        }
        ...
        if (!recursedProperty) {
          ...
          if (recursor !== recursorUtils.NOOP_RECURSOR) return new PropertyStats(recursor)
        }
        ...
        if (!recursedIterable) {
          ...
          if (recursor !== recursorUtils.NOOP_RECURSOR) return new IterableStats(recursor)
        }
      }
