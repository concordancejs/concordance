    ...
    return () => {
      ...
      while (!recursedList || !recursedProperty || !recursedIterable) {
        ...
        if (!recursedList && (this.isList || recursedProperty)) {
          ...
          recursedList = true
          if (recursor !== recursorUtils.NOOP_RECURSOR) return new stats.ListStats(recursor)
        }
        ...
        if (!recursedProperty) {
          ...
          if (recursor !== recursorUtils.NOOP_RECURSOR) return new stats.PropertyStats(recursor)
        }
        ...
        if (!recursedIterable) {
          ...
          if (recursor !== recursorUtils.NOOP_RECURSOR) return new stats.IterableStats(recursor)
        }
      }
