...
class ComplexStats {
  ...
  compare (expected) {
    return expected.isComplexStats &&
...
class Complex {
  ...
  createListRecursor () {
    if (!hasLength(this.instance)) return null
    ...
    const next = () => {
      ...
      const current = index
      index++
      const value = this.describeNested(this.instance[current])
  ...
  createIterableRecursor () {
    if (!this.instance[Symbol.iterator]) return null

    const ignoreIfAccessor = hasLength(this.instance)
    ...
    const next = () => {
      ...
      if (!iterator) {
        iterator = this.instance[Symbol.iterator]()
      }
