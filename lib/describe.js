...
function describeComplex(value, registry, tryPlugins, describeAny, describeItem, describeMapEntry, describeProperty) {
	...
	if (describeValue === null) {
		if (SpecializedComplexes.has(stringTag)) {
			...
			describeValue = unboxed === null ? objectValue.describe : boxedValue.describe;
		}
	}
...
const describeItem = (index, valueDescriptor) => valueDescriptor.isPrimitive === true
	? itemDescriptor.describePrimitive(index, valueDescriptor)
	: itemDescriptor.describeComplex(index, valueDescriptor);

const describeMapEntry = (keyDescriptor, valueDescriptor) => mapEntryDescriptor.describe(keyDescriptor, valueDescriptor);
...
export default function describe(value, options) {
	...
	const curriedComplex = c => describeComplex(c, registry, tryPlugins, describeAny, describeItem, describeMapEntry, describeProperty);
