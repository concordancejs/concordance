...
// Adopted from https://github.com/facebook/jest/blob/9874b81fb51a1c8201d2ed8be1785682c25d65e2/packages/pretty-format/perf/test.js
...
function test (name, value, ignoreResult) {
  const prettyFormatted = testCase(
    'prettyFormat()  ',
    () => prettyFormat(value)
  )

  const formatted = testCase('format()        ', () => format(value))
  ...
  const results = [prettyFormatted, formatted, inspected, stringified].sort((a, b) => {
